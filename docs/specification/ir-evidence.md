...
An alignment cell may combine a source endpoint with an author macro callsite when its own constructor span and source-backed alignment establish ownership. The writer requires one source identity, ordered nonempty intervals, and an expansion origin consistent with the callsite. A leading invocation and its arguments must end before the recorded cell boundary; a trailing invocation must not have arguments beyond its recorded callsite edge. Generated delimiters, macro-generated alignments, inherited carrier spans and unrelated invocations do not establish this interval. Unjustified mixed endpoints remain explicitly unlocated; the writer never reconstructs them from serialized `tex`. The `macro-endpoint` fixture pins both repaired cells and unchanged controls.

Titles, item labels, captions and equation tags replay the occurrences of their already-read arguments. Nested `$...$` math owns those author delimiters even when one argument is digested into several title or reference tags. Conditional selection and delimited argument readers retain token occurrences; the item and amsmath bindings explicitly distinguish stored arguments from ordinary author macro templates. A source range that merely contains the formula, such as the surrounding `\caption` command, is insufficient. The `replay-arguments` fixture checks literal ownership, independent byte ranges, repeated identical labels at distinct locations, grouped and delimited arguments, and unchanged stripped output. Whole-formula author macros remain callsite-only.

On a callsite-only carrier, `capture:macro` names the control-sequence token of its recorded author invocation, including the leading backslash (for example `\eq`). The name is fixed when that author frame is created and survives nested helper expansion. A `\let` alias keeps the invoked alias; redefinitions may share the same name. Replayed argument tokens retain their own invocation identity. The attribute is absent when the author frame has no control-sequence token, and on source, cross-source or unlocated carriers. It neither changes the callsite byte range nor identifies an installed definition or expanded inner run. The `macro-names` fixture pins these contrasts; repaired tag and subfloat arguments remain source-owned without this attribute.
