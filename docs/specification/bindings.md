...
| **semantic** | the package contributes notation or inline carriers with meaning and no object of their own: arrows, relations, stacked or scaled symbols, index layout, links | emit tokens with `role`, and `meaning` or `name` where the package fixes one; parse without `parse_kludge`; keep notational distinctions as traces; revert to valid source syntax | flatten notation to a glyph with no role; invent a meaning the package does not define; drop a distinction because the consumer might not need it |
