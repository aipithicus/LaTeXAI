...
The byte-map controls also check strict UTF-8 width boundaries against independently encoded characters, genuine replacement scalars adjacent to malformed groups, and the relaxed `utf8` codec's five-byte values. The optimized strict path must preserve every unit span and substitution event while leaving the other codecs on their existing path.
...
`t/49_line_spans.t` checks explicit grapheme byte intervals, including combining marks, emoji sequences, malformed groups and encoding changes. It also checks that retained line spans, substitution events and separate Mouth transformations remain independently owned.
...
Without projection, any stripped-document difference fails. An explicit hash-pinned projection verifies the top-level search-path processing instruction against each receipt's actual CLI arguments. It interprets the repository's former preload directory and current `scripts/preloads` as the same logical role, and resolves the recorded absolute/relative entrypoint and job-local output addresses. Search order, other paths, preloads and compiler options must still agree. Missing, duplicate, malformed or inconsistent search-path instructions fail.

Runtime projection version 2 also maps the verified source-root prefix in top-level `graphicspath` instructions to `article-source`. The graphics binding emits absolute directories relative to that source root. Directory suffixes, order, multiplicity and external paths remain observable; relative or malformed graphics metadata fails. Each instruction's before/after text is retained. `t/99_capture_runtime.t` covers copied roots, changed directories, siblings, external paths, ordering and duplicates. Other PIs, comments, whitespace and manuscript nodes are untouched. This is an audit interpretation of execution metadata; runtime paths and the engine serializer are unchanged. A contract change requires new comparison records while preserving the earlier verdicts and projections.
