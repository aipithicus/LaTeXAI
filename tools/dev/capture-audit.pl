...
use CaptureAudit qw(snapshot write_json write_raw finish_run);
use IPC::Run3;

my $mode = shift @ARGV || '';
my ($output, $baseline, $legacy);
my $jobs = 1;
GetOptions('output=s' => \$output, 'baseline=s' => \$baseline, 'legacy-off=s' => \$legacy, 'jobs=i' => \$jobs)
  or die "Invalid audit arguments\n";
die "usage: $0 record|compare --output NEW_DIR [--baseline DIR] [--legacy-off DIR] [t/driver.t ...]\n"
...
my $run = finish_run($output, \@drivers, $baseline, $started && !$status ? 0 : 1, $before, $after);
