...
- Fallback without the aliases: `& "$env:PERL_ROOT\perl\bin\perl.exe" -I lib bin/latexml --log=temp/logs/<job>.latexml.log …`. That path is unbounded; the aliases go through `Invoke-LaTeXAINative` with finite policy timeouts (`scripts/policy.psd1`: Direct 300 s, Test 900 s, Markdown 900 s, Gauntlet native/job 3600 s, gauntlet batch wait 28800 s). `LATEXAI_NATIVE_TIMEOUT` or an explicit `0` is the unbounded diagnostic.
- `ltbatch` and `lgauntlet` import the batch executor at `CDXSCI_ROOT`. That checkout must load (7.5+; current landed executor is `15b064ca`). `lcfg` / `scripts/test-run.ps1 -Preview` / `scripts/gauntlet-run.ps1 -Preview` report resolved runtimes, budgets and job addresses without starting work. `-Package` with `-EvidenceDirectory` is observed-route selection only.
