...
sub _captureMixedCellIsSource {
  my ($self, $node, $box, $span, $gullet, $start_kind, $start, $end_kind, $end) = @_;
  return 0 unless $start && $end
    && (($start_kind eq 'callsite' && $end_kind eq 'source')
      || ($start_kind eq 'source' && $end_kind eq 'callsite'));

  # A cell's math delimiters come from the alignment template. Its own
  # constructor records the first content occurrence and the consumed cell
  # boundary. An inherited span, an ordinary generated '$', or an alignment
  # generated by another author invocation does not establish that ownership.
  my $own_span = $box->can('getProperty') && $box->getProperty('captureSpan');
  return 0 unless $own_span && $own_span == $span;
  my $parent = $node->parentNode;
  return 0 unless $parent && getNodeQName($self, $parent) eq 'ltx:td';
  my $owner = getNodeBox($self, $parent);
  return 0 unless $owner && $owner->isa('LaTeXML::Core::Alignment');
  my $owner_span = $owner->getProperty('captureSpan');
  return 0 unless $owner_span;
  my ($owner_kind, $owner_start) = $gullet->resolveOccurrence($$owner_span{startOccurrence});
  return 0 unless $owner_kind eq 'source' && $owner_start;

  my $callsite = $start_kind eq 'callsite' ? $start : $end;
  my $generated = $start_kind eq 'callsite' ? $$span{startOccurrence} : $$span{endOccurrence};
  my $source = $start_kind eq 'source' ? $$span{startOccurrence} : $$span{endOccurrence};
  my ($source_kind) = $gullet->resolveOccurrence($source);
  my ($origin_kind, $origin) = $gullet->resolveOccurrence($$generated{origin});
  return 0 unless $source_kind eq 'source' && $origin_kind eq 'source' && $origin
    && defined $$callsite{authorFrameId};
  for my $occurrence ($start, $end, $owner_start, $origin) {
    return 0 unless defined $$occurrence{sourceId} && defined $$start{sourceId}
      && $$occurrence{sourceId} eq $$start{sourceId};
    for my $edge (qw(byteStart byteEnd)) {
      return 0 unless defined $$occurrence{$edge} && $$occurrence{$edge} =~ /^\d+$/; }
    return 0 unless $$occurrence{byteStart} < $$occurrence{byteEnd};
  }
  return 0 unless $$owner_start{byteEnd} <= $$start{byteStart}
    && $$start{byteEnd} <= $$end{byteStart}
    && $$origin{byteStart} == $$callsite{byteStart}
    && $$origin{byteEnd} >= $$callsite{byteEnd};
  # A leading invocation may have arguments inside the cell. At the trailing
  # edge, a control-sequence-only callsite must not truncate consumed arguments.
  return $start_kind eq 'callsite'
    ? $$origin{byteEnd} <= $$end{byteStart}
    : $$origin{byteEnd} == $$callsite{byteEnd}; }
...
sub _captureMath {
  ...
  if (_captureMixedCellIsSource($self, $node, $box, $span, $gullet,
      $start_kind, $start_occurrence, $end_kind, $end_occurrence)) {
    ($start_kind, $end_kind) = ('source', 'source'); }
